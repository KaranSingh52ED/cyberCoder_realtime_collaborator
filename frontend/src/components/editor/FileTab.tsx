...
function FileTab() {
    const { openFiles, closeFile, activeFile, updateFileContent, setActiveFile } = useFileSystem()
    ...
    return (
        <div className="flex h-[50px] w-full select-none gap-2 overflow-x-auto p-2 pb-0" ref={fileTabRef}>
            {openFiles.map((file) => (
                <span
                    key={file.id}
                    className={cn('flex w-fit cursor-pointer items-center rounded-t-md px-2 py-1 text-white', {
                        'bg-darkHover': file.id === activeFile?.id,
                    })}
                    onClick={() => changeActiveFile(file.id)}
                >
                    <Icon icon={getIconClassName(file.name)} fontSize={22} className="mr-2 min-w-fit" />
                    <p className="flex-grow cursor-pointer overflow-hidden truncate" title={file.name}>
                        {file.name}
                    </p>
                    <IoClose
